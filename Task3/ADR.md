...
| 3	| Клиент |	Просмотр доступного списка депозитов	| Клиент видит актуальные депозитные продукты с условиями (ставки, сроки).|
| 4  | Кол-центр                      | Обработка заявки с сайта          | Менеджер кол-центра связывается с клиентом для уточнения условий.        |
| 5  | Бэк-офис                      | Подтверждение заявки в АБС        | Сотрудник бэк-офиса проверяет и подтверждает заявку → отправка СМС клиенту. |
| 6  | АБС                           | Расчёт ставок                     | Автоматический расчёт ставок на основе данных из АБС (вместо Excel).     |
